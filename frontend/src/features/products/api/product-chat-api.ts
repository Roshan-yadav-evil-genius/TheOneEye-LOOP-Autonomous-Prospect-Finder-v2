...
export const productChatApi = {
  getHistory: async (productId: string) =>
    (await apiClient.get<ChatHistoryRead>(`/api/v1/products/${productId}/chat/history`)).data,

  clearChat: async (productId: string) =>
    await apiClient.delete(`/api/v1/products/${productId}/chat`),
