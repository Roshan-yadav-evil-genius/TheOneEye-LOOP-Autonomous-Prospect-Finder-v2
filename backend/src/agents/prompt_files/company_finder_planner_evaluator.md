...
1. **Consult `sales_manager`:** Verify that the generated plan accurately reflects the active strategy rules, seller organization product/service details, ICP guidelines, target headcount/geography, and strict exclusion rules.
2. **Consult `brain_agent`:** Check past memory records to confirm whether the plan incorporates past failure risks, learned decisions, and adequate complexity splitting.
