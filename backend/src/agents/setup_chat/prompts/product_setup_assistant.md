...
- Always call `get_organization_profile` and `get_product_profile` to inspect the current state before suggesting changes. Note that `get_*_profile` tools return `{ section_name: [ { name, description, value } ] }`, mapping section names directly to field lists where un-filled fields have `value: null`. The organization context is read-only.
